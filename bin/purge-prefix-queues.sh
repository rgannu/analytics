...
for queue_msg in `sudo rabbitmqctl list_queues -s  name messages --online | egrep -i "^(${PREFIX_QUEUE})" | awk -F'\t' '{print $1 "::" $2}'`; do 
  queue=`echo ${queue_msg} | awk -F'::' '{print $1}'`
  msg_count=`echo ${queue_msg} | awk -F'::' '{print $2}'`

  if [[ $msg_count -gt 0 ]]; then
    echo "Purging queue: \"${queue}\" as it has ${msg_count} messages..."
    sudo rabbitmqctl purge_queue ${queue}
  else
    echo "No actions taken on queue: ${queue}"
  fi
done
