...
rabbitmqctl list_queues -s  name messages --online | egrep -i "^[-]?([0-9])|(analytics.unifly|analytics.navcan|analytics-schema-registry.navcan)" > /tmp/rabbitmq_queues_info
...
while IFS= read -r line; do
  ...
  if [[ ${msg_count} -gt 0 ]]; then
    ...
    echo "rabbitmqctl purge_queue ${queue}"
  fi

  if [[ ${queue} =~ [0-9] ]]; then
    #echo "Deleting random queue \"${queue}\""
    echo "rabbitmqctl delete_queue ${queue}"
  fi 
